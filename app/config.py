...
_DEFAULTS = {
    ...
    "ai_proofread_prompt": (
        "以下は怪談本文をMeCab形態素解析で機械的にひらがな化したものです。"
        "助詞「は→わ」「へ→え」変換やほぼ全てのひらがな読みは既に確定しています。\n\n"
        "あなたの仕事は **漢字部分の読み修正だけ** です。\n\n"
        "## ルール (簡潔)\n"
        "1. **入力にあるひらがな・カタカナ・記号・改行は1文字も変更しない**。"
        "は/わ/ば/ぱ/etc. の出現は全て既に正しい。AIが触ると壊れる。\n"
        "2. **漢字の読みが間違っている場合のみ、その漢字をひらがなに置き換える**。"
        "(例: 「四体」が「したい」と読まれる文脈なら「よんたい」に直す)\n"
        "3. **修正不要なら入力を一字一句そのまま返す**。\n"
        "4. 説明・前置き・コードブロックを含めず、本文のみ返す。\n\n"
        "## 出力に絶対含めてはいけない文字\n"
        ...
        "## 判断基準\n"
        "「ひらがなを直したい」と思ったら、それは **間違い**。手を止めて入力をそのまま返す。\n"
        "漢字の読みだけが対象。\n\n"
        "参照用の原文(漢字混じり):\n"
        "---\n"
        "{raw}\n"
